...
dt.tests = rbind(data.table(expand.grid(c('power_decay', 'const_pos', 'const_neg'), 3:8, 1:nrep)))
...
print(dt.results[, lapply(.SD, mean), keyby=.(mode, amp, method)])
saveRDS(dt.results, 'dt.resultsGaussianSmall.RDS')
...
dt.results = rbindlist(lst.results)
saveRDS(dt.results, 'dt.resultsGaussianLarge.RDS')
